...
ChiperData SHA256::encrypt(const ChiperData &data) {
...
ChiperData SHA256::hash() {
    ChiperData hash(32, 0);
...
void SHA256::transform() {
    uint32_t maj, xorA, ch, xorE, sum, newA, newE, m[BLOCK_SIZE];
    ...
    for (uint8_t k = 16 ; k < BLOCK_SIZE; k++) { // Remaining 48 blocks
...
void SHA256::pad() {
    ...
    uint8_t end = m_blocklen < 56 ? 56 : BLOCK_SIZE;
...
void SHA256::revert(ChiperData &hash) {
